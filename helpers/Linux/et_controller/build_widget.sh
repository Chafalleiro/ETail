...
echo "🚀 Building etail-monitor-widget Executable"
echo "=============================================="
...
--add-data "$HOME/.config/etail-monitor-controller/*:.config/etail-monitor-controller" \
etail-monitor-widget.py

# Check if build was successful
if [ -f "./dist/etail-monitor-controller" ]; then
    echo "✅ Build successful!"
    echo "📦 Executable: ./dist/etail-monitor-widget"
    
    # Make executable
    chmod +x ./dist/etail-monitor-widget
    
    # Test the executable
    echo "🧪 Testing executable..."
    ./dist/etail-monitor-widget --help 2>/dev/null && echo "✅ Executable test passed" || echo "⚠️  Executable may have issues"
else
    echo "❌ Build failed!"
    exit 1
fi
